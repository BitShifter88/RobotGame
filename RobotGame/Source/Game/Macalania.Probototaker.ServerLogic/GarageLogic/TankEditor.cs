    ...
    class TankEditor : GameObject
    {
        ...
        private Point ManipulateComponent(Point mousePos)
        {
            ...
            if (MouseInput.IsLeftMouseClicked())
            {
                ...
                {
                    TurretBrick tb = new TurretBrick(Tank, Room);
            ...
            if (KeyboardInput.IsKeyClicked(Keys.R))
            {
                if (_selectedModule != null && _selectedModule.PluginDir != PluginDirection.NonDirectional)
                {
                    ...
                    while (true)
                    {
                        ...
                        {
                            TurretModule module = TurretModule.GenerateTurretModule(_selectedModule.PluginType, (PluginDirection)currentDir, Room);
                            module.Load(Room.Content);
                            module.SetTank(Tank);
        ...
        private void SelectComponent()
        {
            ...
            {
                _selectedModule = null;
                _selectedBrick = new TurretBrick(Tank, Room);
                _selectedBrick.Load(Room.Content);
            ...
            {
                _selectedBrick = null;
                _selectedModule = new ArtileryStarter(Room);
                _selectedModule.Load(Room.Content);
            }
            else if (KeyboardInput.IsKeyClicked(Keys.NumPad2))
            {
                _selectedBrick = null;
                RocketStarterPlugin rsp = new RocketStarterPlugin(Tanks.Plugins.PluginDirection.Right, Room);
                rsp.SetTank(Tank);
                rsp.Load(Room.Content);
                rsp.ReloadRocket();
                _selectedModule = rsp;
            }
            ...
            {
                _selectedBrick = null;
                _selectedModule = new MiniCanon(Room);
                _selectedModule.Load(Room.Content);
            }
        }
